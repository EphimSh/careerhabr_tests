...
graph TD
    ...
    D -->|Зарплата| G
    D -->|Город| H
    E -->|Просмотр вакансий| I
    F -->|Просмотр вакансий| I
    G -->|Просмотр вакансий| I
    H -->|Просмотр вакансий| I
    I -->|Выбор подходящей вакансии| J(Подать заявку)
    J -->|Заполнить анкету| K(Отправить заявку)
    K -->|Подтверждение заявки| L
    L -->|Успешно подана заявка| M(Закрыть)
    L -->|Ошибка при подаче| N(Связаться с поддержкой)
```
